...
| D36 | **(29/07) Thiết kế lại PHÂN CẤP 3 màn** — Ghi số: `luutrinh` mở sẵn, `baome`/`baocan`/`suco` gập trong `<details>` (bảng `GAP` trong `views/ghiso.js`). Vào hộp: lưới công nhân tách *chưa nhập* (ô to) / *đã nhập* (ô nhỏ + số hộp), `paint()` = `veBang()` + `veNV()` để không lệch. Quản lý: cảnh báo gộp 1 khối "N việc cần xử lý", có cả trạng thái BÌNH THƯỜNG. | D35 chỉ sửa lỗi và giữ nguyên bố cục nên nhìn như không đổi gì; D36 mới là đổi cấu trúc |
| D35 | **(29/07) Viết lại toàn bộ `shell.css` theo skill `ux-ui-design`** — hệ token đầy đủ (màu theo vai trò, thang 4/8, 6 nấc chữ, 3 nấc vùng chạm) + dark theme. Sửa 4 lỗi thật: `--sx-line` dùng 11 lần chưa từng khai báo (viền không render); 4/6 màu ngữ nghĩa trượt WCAG AA (chữ phụ 2.9:1); không có focus ring; vùng chạm 40px. Kèm `aria-label`/`aria-live`, `tabular-nums`, `prefers-reduced-motion`. **Mọi tên class giữ nguyên** → không sửa một dòng logic card nào. | Verify bằng chính script của skill: `check-contrast.py --palette` 0 lỗi, `audit-ui.sh` còn 8 dòng đều là false positive đã giải thích |
| D34 | **(29/07) Lưu đồ tồn BTP vẽ theo khuôn lưu đồ tầng 1** (ô *nhãn → tổng kg to → chi tiết*, mũi tên nối, ô có hàng thì nổi) và **ẩn loại tồn 0**. Tồn âm VẪN hiện, viền ô đỏ. Mỗi chặng trả thêm `tong` + `so_loai`; item sort giảm dần theo tồn. | Nhìn một nhịp với tầng 1; phần đang có hàng không bị 8 loại rỗng làm chìm |
| D33 | **(29/07) Hai màn nhập liệu chỉ giữ việc phải gõ.** `chotngay` + `luutrinhbtp` chuyển sang view **quanly**; `quanly` từ view standalone thành view **vừa dashboard vừa lắp card** (card nằm ĐẦU màn, trên phần theo dõi). `CARD_ROLES["chotngay"] = [QUAN_LY]` — **QC#2 không còn chốt ngày** (đảo D9). Bỏ bảng "Tồn BTP hiện tại" ở dashboard (thẻ lưu đồ thay thế, còn hơn: theo luồng + quy ra mẻ); giữ banner cảnh báo tồn âm. Lưu đồ BTP thêm chặng đầu **Đỗ ở xưởng** (BTP-Dau tại kho Xưởng) để phủ đúng phần bảng cũ hiện. | Màn ghi số còn 4 thẻ, màn vào hộp còn 2. Người nhập ≠ người chốt sổ — siết đúng hướng §2.1 |
| D32 | **(29/07) Lưu đồ tồn BTP tầng 2/3** (`sx.api.portal.luu_do_btp`, card `luutrinhbtp`). 2 nhánh: *Bánh đậu xanh* = Bột nền + Đường hoán → Bột bánh → TP; *Bột đậu* = Bột nền → Bột đậu → TP. **CHỈ ĐỌC** — tầng 2/3 không có nút công đoạn như tầng 1 vì bột bánh/bột đậu sinh khi báo mẻ và bị trừ lúc TP vào hộp (backflush — D8). Bột bánh/bột đậu hiện thêm **quy ra số mẻ** theo `BOM.custom_co_me_chuan_kg`. TP gộp 1 số + đếm SKU còn hàng (liệt kê hết thì dài vô ích). Phân TP về nhánh theo **BOM tầng 3** (RM là BTP-Banh hay BTP-Bot-SP), không đoán theo tên SKU. | QC nhìn ra hàng đang đọng khúc nào trước khi quyết định hôm nay trộn gì |
| D31 | **(29/07) Luồng công đoạn tầng 1 hiện rõ trên portal + có tồn BTP.** Thêm 2 BTP mỗi loại đỗ: **Đỗ ủ**, **Đỗ vỡ** (nhóm `BTP-Dau`, batch `{lô R}-U` / `{lô R}-V`). Kho mới **Xưởng sản xuất** (`SX Settings.kho_xuong`, trống thì dùng Kho BTP). Luồng: *Xuất kho đỗ* (Material Transfer NVL→Xưởng, FIFO lô NCC, sinh lô R) → *Luộc + rang* → Đỗ ủ → *Tách vỏ* (hoàn tất cả lô **hoặc** nhập kg cụ thể) → Đỗ vỡ → *Nghiền bột* → Bột nền (Kho BTP, batch = lô R). Mỗi công đoạn = **Stock Entry Repack**, kg ra do QC cân thật — KHÔNG cần BOM cho các chặng này. Tồn từng chặng đọc thẳng từ Bin/Batch, không có state phụ. **Thay D2 (đỗ rang/đỗ vỡ không còn là WIP vô hình), thay D7/D18 (chốt ngày KHÔNG tự nhập bột nữa — chỉ cảnh báo lô còn ở xưởng).** | Nhìn lưu đồ biết ngay đang dở dang ở chặng nào, bao nhiêu kg; hao hụt từng công đoạn ghi nhận được |
| D30 | **(29/07) Chấm ĂN CA / ĂN ĐÊM ngay trên bảng vào hộp.** Child `SX Bang Vao Hop An Ca` (nhân viên × 2 ô tick). Chốt ngày ghi vào **đúng dòng ngày** của phiếu lương khoán: field kiểu **Check** → đánh dấu 1/0 (tiền do bên lương quy đổi); field kiểu **số** → ghi số tiền lấy từ `SX Settings.tien_an_ca / tien_an_dem` và cộng vào `thunhapngay`. Người CHỈ được chấm ăn (không vào hộp) vẫn ra dòng lương. Chốt lại = ghi đè cả 2 ô. **Thay thế phần "không đụng tienanca/andem" của D22.** | QC chấm 1 lần cùng lúc với sản lượng; bên lương khỏi nhập tay |
